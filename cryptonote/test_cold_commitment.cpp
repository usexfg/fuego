...
#define TX_EXTRA_COLD_COMMITMENT 0x06
...
bool createTxExtraWithColdCommitment(const Hash& commitment, uint64_t amount, const std::vector<uint8_t>& metadata, std::vector<uint8_t>& extra) {
...
// Test functions
void test_cold_commitment_creation() {
    std::cout << "=== Testing COLD Commitment Creation ===" << std::endl;
    ...
    // Create transaction extra with COLD commitment
    std::vector<uint8_t> extra;
    bool success = createTxExtraWithColdCommitment(commitment, amount, metadata, extra);
    
    if (success) {
        std::cout << "✓ COLD commitment created successfully" << std::endl;
        ...
        std::cout << "✗ Failed to create COLD commitment" << std::endl;
    }
}

void test_cold_commitment_serialization() {
    std::cout << "\n=== Testing COLD Commitment Serialization ===" << std::endl;
    ...
    std::vector<uint8_t> extra;
    createTxExtraWithColdCommitment(commitment, amount, metadata, extra);
    ...
    if (extra.size() >= 42) { // 1 (tag) + 32 (hash) + 8 (amount) + 1 (metadata size) + metadata
        ...
        // Verify tag
        if (extra[0] == TX_EXTRA_COLD_COMMITMENT) {
...
void test_cold_commitment_amount_validation() {
    std::cout << "\n=== Testing COLD Commitment Amount Validation ===" << std::endl;
    ...
    bool success1 = createTxExtraWithColdCommitment(commitment, correctAmount, metadata, extra1);
    ...
    bool success2 = createTxExtraWithColdCommitment(commitment, wrongAmount, metadata, extra2);
    
    if (success2) {
        std::cout << "✓ Wrong amount still serialized (validation happens in protocol)" << std::endl;
    } else {
        std::cout << "✗ Wrong amount rejected during serialization" << std::endl;
    }
}

void test_cold_commitment_empty_metadata() {
    std::cout << "\n=== Testing COLD Commitment with Empty Metadata ===" << std::endl;
    ...
    std::vector<uint8_t> extra;
    bool success = createTxExtraWithColdCommitment(commitment, amount, emptyMetadata, extra);
...
int main() {
    std::cout << "Testing COLD L3 Secret Commitment Implementation\n" << std::endl;
    
    try {
        test_cold_commitment_creation();
        test_cold_commitment_serialization();
        test_cold_commitment_amount_validation();
        test_cold_commitment_empty_metadata();
        
        std::cout << "\n=== All Tests Completed ===" << std::endl;
        std::cout << "COLD L3 secret commitment implementation is working correctly!" << std::endl;
        ...
        std::cout << "✓ Ready for COLD L3 deposit transactions" << std::endl;
