...
//  ELDERNODE RELAYER ENDPOINTS

struct StartEldernodeMonitoring
{
  struct Request
  {
    std::string bridgeContractAddress;  // Arbitrum bridge contract address
    std::string arbitrumRpcUrl;         // Arbitrum RPC endpoint
    std::string networkId;              // Fuego network identifier
...
struct StopEldernodeMonitoring
{
  struct Request
  {
    void serialize(CryptoNote::ISerializer &serializer);
...
struct GetEldernodeStatistics
{
  ...
  struct Response
  {
    bool success;
    bool isMonitoring;
    uint64_t totalProofsProcessed;
    uint64_t totalProofsSubmitted;
    uint64_t totalProofsFailed;
    uint64_t lastProcessedBlock;
    std::string status;
    std::string errorMessage;
...
struct GenerateMerkleProof
{
  ...
  {
    bool success;
    std::string merkleProof;  // JSON encoded Merkle proof
    std::string errorMessage;
...
struct GetValidationProof
{
  ...
  {
    bool success;
    std::string validationProof;  // JSON encoded validation proof
    std::string errorMessage;
...
struct SubmitToBridge
{
  struct Request
  {
    std::string starkProof;      // Base64 encoded STARK proof
    std::string validationProof; // JSON encoded validation proof from Eldernode
    std::string recipientAddress; // Arbitrum address to receive HEAT
  ...
  {
    bool success;
    std::string bridgeResponse;   // Response from Arbitrum bridge
    std::string errorMessage;
