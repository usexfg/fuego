      ...
      if (upgradeHeight == UNDEF_HEIGHT) {
        if (m_blockchain.empty()) {
          ...
          if (it == m_blockchain.end() || it->bl.majorVersion != m_targetVersion) {
            logger(Logging::LoggingLevel::ERROR, Logging::BRIGHT_RED) << "Internal error: upgrade height isn't found";
            return false;
          }
          ...
          if (m_votingCompleteHeight == UNDEF_HEIGHT) {
            logger(Logging::LoggingLevel::ERROR, Logging::BRIGHT_RED) << "Internal error: voting complete height isn't found, upgrade height = " << upgradeHeight;
            return false;
          }
        } else {
          m_votingCompleteHeight = UNDEF_HEIGHT;
        ...
        if (m_blockchain.size() <= upgradeHeight + 1) {
          if (m_blockchain.back().bl.majorVersion >= m_targetVersion) {
            logger(Logging::LoggingLevel::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height " << (m_blockchain.size() - 1) <<
          ...
          if (blockVersionAfterUpgradeHeight != m_targetVersion) {
            logger(Logging::LoggingLevel::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height " << (upgradeHeight + 1) <<
