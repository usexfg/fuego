...
#ifdef __aarch64__
  // ARM64: Since we can't use context switching, we need to handle this differently
  // For System::Context, we'll use a thread-based approach
  assert(firstReusableContext == nullptr);
  NativeContext context;
  context.uctx = nullptr;  // ARM64 doesn't use ucontext
  context.interrupted = false;
  context.next = nullptr;
  context.inExecutionQueue = false;
  firstReusableContext = &context;

  // On ARM64, we execute the procedure directly in this thread
